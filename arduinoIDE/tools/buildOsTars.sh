...
cp -r efm_upload/linux efm_tools-1.0/efm_upload/linux
cp -r EFM_Serial2Mqtt/linuxArm32 efm_tools-1.0/EFM_Serial2Mqtt/linux

tar czvf efm_tools-1.0-linuxArm32.tar.gz efm_tools-1.0
...
#Arm32
echo { >> modules.json
echo \"url\": \"http://engimusing.github.io/arduinoIDE/tools/efm_tools-1.0-linuxArm32.tar.gz\", >> modules.json
echo -n \"checksum\": \"SHA-256: >> modules.json    
sha256sum efm_tools-1.0-linuxArm32.tar.gz | grep -Eo '^[^ ]+' | tr -d '\n' >> modules.json
echo \", >> modules.json
echo \"host\": \"arm-linux-gnueabihf\", >> modules.json
echo \"archiveFileName\": \"efm_tools-1.0-linuxArm32.tar.gz\", >> modules.json
echo -n \"size\": \" >> modules.json
wc -c efm_tools-1.0-linuxArm32.tar.gz | grep -Eo '^[^ ]+' | tr -d '\n' >> modules.json
echo \" >> modules.json
echo }, >> modules.json
